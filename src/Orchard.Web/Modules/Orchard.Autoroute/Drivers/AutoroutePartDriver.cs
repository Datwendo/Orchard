...
namespace Orchard.Autoroute.Drivers {
    public class AutoroutePartDriver : ContentPartDriver<AutoroutePart> {
        ...
        // CS 3/6
        protected override DriverResult FrontEditor(AutoroutePart part, string editType, dynamic shapeHelper) {
            return FrontEditor(part, editType, null, shapeHelper);
        }
        // CS 3/6
        protected override DriverResult FrontEditor(AutoroutePart part, string editType, IUpdateModel updater, dynamic shapeHelper) {
            var settings = part.TypePartDefinition.Settings.GetModel<AutorouteSettings>();
            var itemCulture = _cultureManager.GetSiteCulture();
            ...
            return ContentShape("Parts_Autoroute_FrontEdit",
                () => shapeHelper.FrontEditorTemplate(TemplateName: "Parts.Autoroute.FrontEdit", Model: viewModel, Prefix: Prefix));
        }
