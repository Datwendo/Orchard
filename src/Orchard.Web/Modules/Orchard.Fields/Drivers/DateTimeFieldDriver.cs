...
namespace Orchard.Fields.Drivers {
    public class DateTimeFieldDriver : ContentFieldDriver<DateTimeField> {
        ...
        // CS 3/6
        protected override DriverResult FrontEditor(ContentPart part, DateTimeField field, string editType, dynamic shapeHelper) {
            var settings = field.PartFieldDefinition.Settings.GetModel<DateTimeFieldSettings>();
            var value = part.IsNew() ? settings.DefaultValue : field.DateTime;
            var options = new DateLocalizationOptions();
            ...
            return ContentShape("Fields_DateTime_FrontEdit", GetDifferentiator(field, part),
                () => shapeHelper.FrontEditorTemplate(TemplateName: TemplateName, Model: viewModel, Prefix: GetPrefix(field, part)));
        }
        // CS 3/6
        protected override DriverResult FrontEditor(ContentPart part, DateTimeField field, string editType, IUpdateModel updater, dynamic shapeHelper) {
            var viewModel = new DateTimeFieldViewModel();
