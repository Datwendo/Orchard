...
namespace Orchard.Fields.Drivers {
    public class EnumerationFieldDriver : ContentFieldDriver<EnumerationField> {
        ...
        // CS 3/6
        protected override DriverResult FrontEditor(ContentPart part, EnumerationField field, string editType, dynamic shapeHelper) {
            return ContentShape("Fields_Enumeration_FrontEdit", GetDifferentiator(field, part),
                () => {
                    if (part.IsNew()) {
                        var settings = field.PartFieldDefinition.Settings.GetModel<EnumerationFieldSettings>();
                        if (!String.IsNullOrWhiteSpace(settings.DefaultValue)) {
        ...
        // CS 3/6
        protected override DriverResult FrontEditor(ContentPart part, EnumerationField field, string editType, IUpdateModel updater, dynamic shapeHelper) {
            if (updater.TryUpdateModel(field, GetPrefix(field, part), null, null)) {
                var settings = field.PartFieldDefinition.Settings.GetModel<EnumerationFieldSettings>();
