        ...
        public static Func<ShapePlacementContext, bool> BuildPredicate(Func<ShapePlacementContext, bool> predicate, KeyValuePair<string, string> term) {
            ...
            switch (term.Key) {
                ...
                case "EditType":
                    ...
                    return ctx => {
                        var edt = ctx.EditType;
                        bool result= (string.Equals(edt, expression, StringComparison.InvariantCulture) ||
                            (string.IsNullOrEmpty(edt) && string.Equals(expression, "Detail", StringComparison.InvariantCulture)))
                            && predicate(ctx);
                            return result;
                            };
                case "Path":
